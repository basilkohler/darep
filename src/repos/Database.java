...
public class Database {
	...
	/*loads(/creates if 'reposPath' doesnt exits) a db in 'reposPath'
	 * doesnt open any files/folders just loads the paths
	 * @param reposPath the location of the repository (parent folder od 'filedb' and 'metadb')
	 */
	...
	* does not check if 'name' is already used, this happens in Repository.add()
	* @param dataset	the path to the dataset to be added
	* @param meta		the metada corresponig to 'dataset'
	* @param copyMode	true if dataset should be copied to the db, else dataset is moved into the db
	*/
